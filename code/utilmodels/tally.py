...
def range_topk(channels, size=1, largest=True):
    '''
    Make a trivial constant topk object that counts
    through indexes in order.
    '''
    rtk = runningstats.RunningTopK(k=size)
    if largest:
        r = torch.arange(size, 0, -1)
    else:
        r = torch.arange(0, size)
    rtk.add(r[:,None].expand(size, channels))
    return rtk

def expand_topk(topk, channels):
    '''
    Expands a topk to have more channels.
    '''
    assert len(topk.data_shape) == 1 and topk.data_shape[0] == 1
    rtk = copy.deepcopy(topk)
    rtk.data_shape = (channels,)
    rtk.top_data = rtk.top_data.expand((channels,
        rtk.top_data.size(1))).clone()
    rtk.top_index= rtk.top_index.expand((channels,
        rtk.top_index.size(1))).clone()
    return rtk
...
def tally_extremek(compute, dataset, sample_size=None,
        batch_size=10, k=100, cachefile=None, **kwargs):
    '''
    ...
    cs = CombinedState(state=cached_state,
            rtk=runningstats.RunningTopK(k=k),
            rbk=runningstats.RunningTopK(k=k, largest=False))
    if cached_state is not None:
        return cs.rtk, cs.rbk
    loader = make_loader(dataset, sample_size, batch_size, **kwargs)
    for batch in pbar(loader):
        sample_tk, sample_bk = call_compute(compute, batch)
        cs.rtk.add(sample_tk)
        cs.rbk.add(sample_bk)
    cs.rtk.to_('cpu')
    cs.rbk.to_('cpu')
...
def tally_extremek_and_quantile(compute, dataset, sample_size=None,
        batch_size=10, k=100, r=4096, cachefile=None, **kwargs):
    '''
    ...
    cs = CombinedState(state=cached_state,
            ...
            rq=runningstats.RunningQuantile(r=r))
    if cached_state is not None:
        return cs.rtk, cs.rbk, cs.rq
    loader = make_loader(dataset, sample_size, batch_size, **kwargs)
    for batch in pbar(loader):
        sample_tk, sample_bk, sample_q = call_compute(compute, batch)
        cs.rtk.add(sample_tk)
        cs.rbk.add(sample_bk)
        cs.rq.add(sample_q)
    cs.rtk.to_('cpu')
...
def tally_cat_dict(compute, dataset, sample_size=None, batch_size=10,
    ...
    cached_state = load_cached_state(cachefile, args)
    if cached_state is not None:
        return torch_from_numpy_dict(cached_state)
    loader = make_loader(dataset, sample_size, batch_size, **kwargs)
...
def tally_covariance(compute, dataset, sample_size=None, batch_size=10,
        split_batch=False, cachefile=None, **kwargs):
    '''
    ...
    loader = make_loader(dataset, sample_size, batch_size, **kwargs)
    rcov = runningstats.RunningCovariance(split_batch=split_batch)
    for batch in pbar(loader):
        sample = call_compute(compute, batch)
        rcov.add(sample)
    rcov.to_('cpu')
...
def tally_cross_covariance(compute, dataset, sample_size=None, batch_size=10,
    ...
    loader = make_loader(dataset, sample_size, batch_size, **kwargs)
    rcov = runningstats.RunningCrossCovariance(split_batch=split_batch)
    for batch in pbar(loader):
...
def tally_second_moment(compute, dataset, sample_size=None, batch_size=10,
    ...
    loader = make_loader(dataset, sample_size, batch_size, **kwargs)
    r2mom = runningstats.RunningSecondMoment(split_batch=split_batch)
    for batch in pbar(loader):
        sample = call_compute(compute, batch)
        r2mom.add(sample)
    r2mom.to_('cpu')
    ...
    def __init__(self, state=None, **kwargs):
        ...
        if state is not None:
            for prefix, obj in self._objs.items():
                obj.set_state_dict(pull_key_prefix(prefix, state))
...
class TensorDict:
    def __init__(self, data=None, state=None):
        if state is not None:
            self.data = torch_from_numpy_dict(state)
        else:
            self.data = data if data is not None else {}
    def state_dict(self):
        return {k: v.detach().cpu().numpy() for k, v in self.data.items()}

def torch_from_numpy_dict(state):
    return {k: torch.from_numpy(v) if len(v.shape) > 0 else v.item()
