...
places_dir = "/users/rsstone/projects_sym/rsstone/unlearning-bias-dev/data/places/"
output_dir = "/users/rsstone/projects_sym/rsstone/unlearning-bias-dev/data/cocoplaces/"
coco_train_annotations_path = "/users/rsstone/projects_sym/rsstone/unlearning-bias-dev/data/coco/annotations/instances_train2017.json"
...
filename = "train-256-places365standard.tar"
"""
# copy over:
print('Copying (could take a while) ...', end='')
original_tarname = f"{places_dir}{filename}"

if not os.path.exists(output_dir + filename):
    subprocess.call(['cp', '-r', original_tarname, output_dir])
print('Done!')

# extract 'em one by one
thing = os.path.join(output_dir, filename)
for place in biased_places:
    output = subprocess.check_output(['tar', '-xvf', thing, '-C', output_dir, 'data_256/{}'.format(place)])
    print(output)
...
######################################################################################
output_subdir = output_dir + "data_256/"
...
for i, target_place in enumerate(unbiased_places):
    L = [f'{target_place}/{filename}' for filename in os.listdir(os.path.join(output_subdir, target_place)) if filename.endswith('.jpg')]
...
coco = COCO(coco_train_annotations_path)
...
for c in range(len(ANOMALIES)):
    ...
    while tr_si < tr_i//NUM_CLASSES:
        ...
        place_img = np.asarray(Image.open(os.path.join(outputs_subdir, place_path)).convert('RGB'))
