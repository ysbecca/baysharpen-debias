...
class CocoOnPlacesDataset(BaseDataset):
    def __init__(self, data_dir='/home/robik/datasets/coco_on_places',
        ...
        h5_file = h5py.File(os.path.join(self.data_dir, f'{self.split}.h5py'), 'r', libver='latest', swmr=True)
    ...
    def __getitem__(self, index):
        ...
        # return item
        return item['x'], item['y'].squeeze(), index
...
def create_coco_on_places_dataset_for_split(dataset_cfg, split, transform, args):
...
def create_coco_on_places_datasets(dataset_cfg, train_transform, test_transform, args):
    split_to_dataset = {'Test': {}}
    eval_splits = ['train', 'idtest', 'validtest', 'oodtest', 'valoodtest', 'sgtest', 'valsgtest', 'anotest']
    eval_aliases = ['train', 'idtest', 'validtest', 'oodtest', 'valoodtest', 'sgtest', 'sgval', 'anotest']
    for eval_split, eval_alias in zip(eval_splits, eval_aliases):
        split_to_dataset['Test'][eval_alias] = create_coco_on_places_dataset_for_split(
                dataset_cfg,
                eval_split,
                test_transform,
                args
            )
    split_to_dataset['Train'] = create_coco_on_places_dataset_for_split(
            dataset_cfg,
            'train',
            train_transform,
            args
